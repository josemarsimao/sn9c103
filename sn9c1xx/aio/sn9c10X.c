...
static int hv7131r_init(struct sn9c102_device* cam)
{
	...
	case BRIDGE_SN9C105:
	case BRIDGE_SN9C120:
		err = sn9c102_write_const_regs(cam, {0x46, 0x01}, {0x40, 0x02},
		                               {0x00, 0x03}, {0x1a, 0x04},
		                               {0x20, 0x05}, {0x20, 0x06},
		                               {0x20, 0x07}, {0x03, 0x10},
		                               {0x08, 0x14}, {0x20, 0x17},
	...
	err += sn9c102_i2c_write(cam, 0x15, 0xe6);
                            ...
                            struct v4l2_control* ctrl)
{
	...
	case V4L2_CID_EXPOSURE:
		{
			int r0, r1, r2;
			if ((r0 = sn9c102_i2c_read(cam, 0x25)) < 0 ||
			    (r1 = sn9c102_i2c_read(cam, 0x26)) < 0 ||
			    (r2 = sn9c102_i2c_read(cam, 0x27)) < 0)
				return -EIO;
			ctrl->value = (r0 << 16) | (r1 << 8) | r2;
		}
		return 0;
                            ...
                            const struct v4l2_control* ctrl)
{
	...
	case SN9C102_V4L2_CID_GREEN_BALANCE:
		...
		break;
	case V4L2_CID_EXPOSURE:
		err += sn9c102_i2c_write(cam, 0x25, ctrl->value >> 16);
		err += sn9c102_i2c_write(cam, 0x26, (ctrl->value&0xff00) >> 8);
		err += sn9c102_i2c_write(cam, 0x27, ctrl->value & 0xff);
		break;
	case V4L2_CID_BLACK_LEVEL:
		{
                            ...
                            const struct v4l2_rect* rect)
{
	...
	int err = 0;
	u8 h_start = (u8)(rect->left - s->cropcap.bounds.left) + 2,
	   v_start = (u8)(rect->top - s->cropcap.bounds.top) + 5;
                                  ...
                                  const struct v4l2_pix_format* pix)
{
	...
	if ((r = sn9c102_i2c_read(cam, 0x01)) < 0)
		return -EIO;
	...
	case BRIDGE_SN9C103:
		if (pix->pixelformat == V4L2_PIX_FMT_SBGGR8) {
			...
			err += sn9c102_i2c_write(cam, 0x01, (r & 0x0f) | 0x04);
		} else {
	...
	case BRIDGE_SN9C120:
		if (pix->pixelformat == V4L2_PIX_FMT_SBGGR8) {
			...
			err += sn9c102_i2c_write(cam, 0x01, (r & 0x0f) | 0x24);
		} else {
...
int sn9c102_probe_hv7131r(struct sn9c102_device* cam)
{
	...
	switch (sn9c102_get_bridge(cam)) {
	case BRIDGE_SN9C103:
		if (sn9c102_write_const_regs(cam, {0x09, 0x01}, {0x44, 0x02},
		                             {0x34, 0x01}, {0x20, 0x17},
		                             {0x34, 0x01}, {0x46, 0x01}))
			return -EIO;
		break;
	case BRIDGE_SN9C105:
	case BRIDGE_SN9C120:
		if (sn9c102_write_const_regs(cam, {0x03, 0xf1}, {0x00, 0xf1},
		                             {0x09, 0x01}, {0x44, 0x02},
		                             {0x34, 0x01}, {0x20, 0x17},
		                             {0x34, 0x01}, {0x46, 0x01}))
			return -EIO;
	...
	if ((devid = sn9c102_i2c_try_read(cam, &hv7131r, 0x00)) < 0)
		return -EIO;
                           ...
                           const struct v4l2_rect* rect)
{
	struct sn9c102_sensor* s = sn9c102_get_sensor(cam);
	int err = 0;
	u8 h_start = 0, v_start = 0;

	switch (sn9c102_get_bridge(cam)) {
	case BRIDGE_SN9C103:
		h_start = (u8)(rect->top - s->cropcap.bounds.top) + 1;
		v_start = (u8)(rect->top - s->cropcap.bounds.top) + 2;
		break;
	case BRIDGE_SN9C105:
	case BRIDGE_SN9C120:
		h_start = (u8)(rect->top - s->cropcap.bounds.top) + 0;
		v_start = (u8)(rect->top - s->cropcap.bounds.top) + 2;
		break;
	default:
		break;
                            ...
                            struct v4l2_control* ctrl)
{
	...
	case V4L2_CID_AUTOGAIN:
		if (sn9c102_i2c_try_raw_write(cam, s, 4, s->i2c_slave_id,
		                              0x01, 0x00, 0x01, 0, 0) ||
		    (sn9c102_i2c_try_raw_read(cam, s, s->i2c_slave_id, 0x06, 2,
		                              data) < 0) ||
		    sn9c102_i2c_try_raw_write(cam, s, 4, s->i2c_slave_id,
		                              0x01, 0x00, 0x04, 0, 0))
			return -EIO;
		ctrl->value = data[1] & 0x02;
		break;
	case V4L2_CID_DO_WHITE_BALANCE:
		if ((r = sn9c102_read_reg(cam, 0x02)) < 0)
			return -EIO;
		ctrl->value = r & 0x04;
		break;
	case V4L2_CID_HFLIP:
                            ...
                            const struct v4l2_control* ctrl)
{
	...
	case V4L2_CID_AUTOGAIN:
		err = sn9c102_i2c_try_raw_write(cam, s, 4, s->i2c_slave_id,
		                                0x01, 0x00, 0x01, 0, 0);
		if (err || (err = sn9c102_i2c_try_raw_read(cam, s,
		                                           s->i2c_slave_id,
		                                           0x06, 2, data) < 0))
			break;
		err += sn9c102_i2c_try_raw_write(cam, s, 4, s->i2c_slave_id,
		                  0x06, (data[0] & 0xbf) | (ctrl->value << 6),
		                  (data[1] & 0xfd) | (ctrl->value << 1), 0, 0);
		err += sn9c102_i2c_try_raw_write(cam, s, 4, s->i2c_slave_id,
		                                 0x01, 0x00, 0x04, 0, 0);
		break;
	case V4L2_CID_DO_WHITE_BALANCE:
...
const struct v4l2_ioctl_ops ioctl_ops = {
    ...
    .vidioc_g_pixelaspect = 0, //sn9c102_vidioc_cropcap,  /* verify */
